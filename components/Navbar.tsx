  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled || isMenuOpen ? 'bg-white/95 dark:bg-black/95 backdrop-blur-md shadow-sm py-3' : 'bg-transparent py-6'
      }`}
    >
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        >
          <div className="relative w-6 h-5">
            <span 
              className={`absolute block h-0.5 w-full bg-current transform transition duration-500 ease-in-out ${
                isMenuOpen ? 'rotate-45 translate-y-2' : ''
              } ${scrolled || isMenuOpen ? 'text-stone-800 dark:text-white' : 'text-white'}`}
            ></span>
            <span 
              className={`absolute block h-0.5 w-full bg-current transform transition duration-500 ease-in-out top-2 ${
                isMenuOpen ? 'opacity-0' : 'opacity-100'
              } ${scrolled || isMenuOpen ? 'text-stone-800 dark:text-white' : 'text-white'}`}
            ></span>
            <span 
              className={`absolute block h-0.5 w-full bg-current transform transition duration-500 ease-in-out top-4 ${
                isMenuOpen ? '-rotate-45 -translate-y-2' : ''
              } ${scrolled || isMenuOpen ? 'text-stone-800 dark:text-white' : 'text-white'}`}
            ></span>
          </div>
      ...
      {/* Mobile Menu Overlay */}
      <div 
        className={`md:hidden absolute top-full left-0 right-0 overflow-hidden transition-all duration-500 ease-in-out ${
          isMenuOpen ? 'max-h-screen opacity-100 visible' : 'max-h-0 opacity-0 invisible'
        } bg-white dark:bg-black border-t border-stone-100 dark:border-stone-900 shadow-xl`}
      >
        <div className="container mx-auto px-6 py-8 flex flex-col space-y-6">
          {navItems.map((item) => (
            <a
              key={item}
              href={`#${item.toLowerCase()}`}
              className="text-lg font-bold tracking-widest text-stone-800 dark:text-stone-200 hover:text-primary transition-colors text-center"
              onClick={() => setIsMenuOpen(false)}
            >
              {item}
            </a>
          ...
          <div className="pt-6 border-t border-stone-100 dark:border-stone-900 flex justify-center space-x-8">
            <a 
              href="https://www.linkedin.com/company/syntrax-software/?viewAsMember=true" 
              target="_blank"
              rel="noopener noreferrer"
